$(document).ready(function () {
  ...
  function playVid() {
    if (video.get(0).paused) {
      video.get(0).play();
      playIcon.css({
        'background': 'url({% static "speech_to_text/img/pause.png %}" )',
      ...
      video.get(0).pause();
      playIcon.css({
        'background': 'url({% static "speech_to_text/img/play.png" %}) ',
        'background-size': '100% 100%'
      });
      overlayScreen.show();
  ...
  function updateplayer() {
    ...
    if (video[0].ended) {
      ...
      overlayScreen.show();
      overlayButton.css({
        'background': 'url({% static "speech_to_text/img/replay.pn %}g ")',
        'background-size': '100% 100%'
      });
    } else if (video[0].paused) {
      overlayButton.css({
        'background': 'url({% static "speech_to_text/img/play-butt %}o n".png)',
        'background-size': '100% 100%'
      });
    }
  }
  ...
  function toggleMute() {
    if (!video[0].muted) {
      video[0].muted = true;
      volume.css({
        'background': 'url({% static "speech_to_text/img/mute.png" %}) ',
        'background-size': '100% 100%'
      });
      intensity.hide();
    } else {
      video[0].muted = false;
      volume.css({
        'background': 'url({% static "speech_to_text/img/volume.pn %}g ")',
        'background-size': '100% 100%'
      });
      intensity.show();
    }
